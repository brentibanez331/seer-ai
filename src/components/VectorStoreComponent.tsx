...
import { EmbeddingNodeProps, VectorStoreNodeProps } from "./interfaces/NodeProps";
import { Slider } from "./ui/slider";
import { EmbeddingNode } from "./interfaces/EmbeddingNode";
import { VectorStoreNode } from "./interfaces/VectorStoreNode";

export const VectorStoreNodeComponent = ({
    node,
    onMouseEnterInput,
    onMouseLeaveInput,
    onMouseDownOutput,
    ...
    y,
    onMouseDownNode,
    onParameterChange }: VectorStoreNodeProps & { node: VectorStoreNode }) => {
    ...
    return (
        ...
        >
            <div className="text-xs px-2 py-2 space-y-4">
                <h2 className="text-md font-bold ">{node.parameters.name} Vector Store</h2>
                <div className="relative">
                    <div className="input-wrapper">
                        <div
                            ...
                            ref={(el) => { }}
                            className="input"
                            onMouseEnter={(event) => handleMouseEnterInput(event.currentTarget, 'embedding')}
                            onMouseLeave={() => handleMouseLeaveInput('embedding')}
                        >
                        </div>
                    </div>
                    <p className="text-neutral-400">Embedding</p>
                </div>
                <div className="relative">
                    <div className="input-wrapper">
                        <div
                ...
                <div className="relative">
                    <div className="output-wrapper">
                        <div
                            ...
                            ref={(el) => { }}
                            className="output"
                            onMouseDown={(event) => handleMouseDownOutput(event.currentTarget, event, "retriever")}
                        >
                        </div>
                    </div>
                    <p className="text-end text-neutral-400">Memory Retriever</p>
                </div>
            </div>
        </div>
    )
